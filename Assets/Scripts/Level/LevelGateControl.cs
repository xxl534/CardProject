...
public class LevelGateControl : MonoBehaviour,IComparable<LevelGateControl>
{
		...
		void OnHover (bool isOver)
		{
				if (_levelInfo.unlocked) {
						if (isOver) {
								HOTween.To (_whiteEdge, _hoverFadeDuration, new TweenParms ().Prop ("alpha", 1f).OnStart (() => {
										_whiteEdge.gameObject.SetActive (true);}));
								HOTween.To (_levelName, _hoverFadeDuration, new TweenParms ().Prop ("effectDistance", new Vector2 (6f, 6f)).OnStart (() => {
										_levelName.effectStyle = UILabel.Effect.Shadow;}));
						} else {
								HOTween.To (_whiteEdge, _hoverFadeDuration, new TweenParms ().Prop ("alpha", 0f).OnComplete (() => {
										_whiteEdge.gameObject.SetActive (false);}));
								HOTween.To (_levelName, _hoverFadeDuration, new TweenParms ().Prop ("effectDistance", Vector2.zero).OnComplete (() => {
...
/// </summary>
		...
		void Init ()
		{
				...
				if (unlock) {
						...
						_levelName.gameObject.SetActive (true);
						_levelName.alpha = 1;
						_gameController.LightPathPoints (_levelIndex - 1);
			collider.enabled=true;
				} else {
