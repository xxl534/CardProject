...
public class AbilityDetailDisplayer : MonoBehaviour
{
	...
	public void DisplayerAbilityDetail(Ability abililty,float displayCoorY)
	{
		DisplayerAbilityDetail(abililty,displayCoorY,-1);
	}
	public void DisplayerAbilityDetail(Ability abililty,float displayCoorY,int cdRound)
	{
		...
		_label_comsume.text="MP:"+abililty.mana.ToString();
		_label_description.text=abililty.GetDescription();

		transform.position=new Vector3(transform.position.x,displayCoorY,transform.position.z);
	...
	{
		HOTween.To(_panel,_fadeTime,new TweenParms().Prop("alpha",0f).Ease( EaseType.Linear).OnComplete(
			delegate() {
			gameObject.SetActive(false);
		}));		
	}
